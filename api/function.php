...
class napthe
{
	...
	{ // hàm gửi card lên api
		...
		); # Các tham số trả ra : check là tham số kiểm tra curl ; msg là messange server trả về ; status là trạng thái ; content là mã bí mật dùng để check card hiệu lực thông thường là 24h
	}
	...
	public function check_card ($content){ // hàm check card trên api
		...
		if (!empty($out->status)){$error = 200;}
		curl_close($ch);
		return array (
		 'check' => $error, // trả về 200 là hoạt động
		 'msg' => $out->msg,
		 'status' => $out->status, // tham số này trả ra nếu bằng 00 thì thẻ đúng ; nếu bằng -10 là thẻ sai ; còn lại là các loại sai khác chi tiết lỗi ở msg
		 'title' => $out->title // thông báo thất bại hay thành công :v
		);
	}
